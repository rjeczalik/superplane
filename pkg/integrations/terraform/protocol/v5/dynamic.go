...
func JSONToDynamicValue(schema tftypes.Type, raw []byte) (*tfprotov5.DynamicValue, error) {
	value, err := valueFromCanonicalJSON(schema, raw)
	if err != nil {
		return nil, fmt.Errorf("decode terraform dynamic JSON: %w", err)
	}

	dynamic, err := tfprotov5.NewDynamicValue(schema, value)
	if err != nil {
...
func DynamicValueToJSON(schema tftypes.Type, value *tfprotov5.DynamicValue) ([]byte, error) {
	if value == nil {
